function [curr_dat1_sz, curr_dat2_sz, curr_dat3_sz, curr_lab_sz] = cafe_store2hdf5(filename, data1, data2, data3, labels, create, startloc, chunksz)  
  ...
  dat3_dims=size(data3);
  lab_dims=size(labels);
  num_samples=dat1_dims(end);
  
  assert(dat2_dims(end)==num_samples, 'Number of samples should be matched between data1 and data2');
  assert(dat3_dims(end)==num_samples, 'Number of samples should be matched between data1 and data3');
  assert(lab_dims(end)==num_samples, 'Number of samples should be matched between data1 and labels');
  ...
  if create
    ...
    h5create(filename, '/data2', [dat2_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [dat2_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/data3', [dat3_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [dat3_dims(1:end-1) chunksz]); % width, height, channels, number 
    ...
    if ~exist('startloc','var') 
      ...
      startloc.dat3=[ones(1,length(dat3_dims)-1), 1];
    ...
    if ~exist('startloc','var')
      ...
      assert(prev_dat1_sz(1:end-1)==dat1_dims(1:end-1), 'Data1 dimensions must match existing dimensions in dataset');
      assert(prev_dat2_sz(1:end-1)==dat2_dims(1:end-1), 'data2 dimensions must match existing dimensions in dataset');
      assert(prev_dat3_sz(1:end-1)==dat3_dims(1:end-1), 'data3 dimensions must match existing dimensions in dataset');
      ...
      startloc.dat3=[ones(1,length(dat3_dims)-1), prev_dat3_sz(end)+1];
